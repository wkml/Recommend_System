...
def hit_ratio(test_num=200):
    ...
    for user in test_users:
        ...
        # index of the last item, the positive one
        x_id = len(x) - 1
...
def train(mode, optimizer, epochs=10, batch_size=128):
    ...
    while ep < epochs:
        ...
        if len(user2items) < batch_size:
            ep += 1
            user2items, item2users, test_user2items = dataset_loader.get_dictionaries()


if __name__ == '__main__':
    dataset_loader = datasets.MovielensDatasetLoader()
    ...
    ncf = NCF.NeuralCollaborativeFiltering(user_num + 1, item_num + 1, embedding_size=32).cuda()
    ncf.join_output_weights()
    print('Hit ratio:', hit_ratio())

    mlp_optimizer = torch.optim.Adam(list(ncf.mlp_item_embeddings.parameters()) +
                                     list(ncf.mlp_user_embeddings.parameters()) +
                                     list(ncf.mlp.parameters()) +
                                     list(ncf.mlp_out.parameters()), lr=1e-3)

    gmf_optimizer = torch.optim.Adam(list(ncf.gmf_item_embeddings.parameters()) +
                                     list(ncf.gmf_user_embeddings.parameters()) +
                                     list(ncf.gmf_out.parameters()), lr=1e-3)

    ncf_optimizer = torch.optim.Adam(ncf.parameters(), lr=5e-4)
